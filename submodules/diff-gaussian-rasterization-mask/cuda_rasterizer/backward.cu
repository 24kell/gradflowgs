...
__global__ void preprocessCUDA(
	...
	const float2* dL_dmask_shunts, // new
	const float2* dL_dmask_signals, // new
...
renderCUDA(
	...
	const float* __restrict__ colors, // mask_procomp
	const float* __restrict__ depths,
	const float* __restrict__ final_Ts,
	const uint32_t* __restrict__ n_contrib,
	const float* __restrict__ dL_dpixels,
	const float* __restrict__ dL_invdepths,
	float2* __restrict__ dL_dmask_shunts, // new
	float2* __restrict__ dL_dmask_signals, // new
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				if (ch == 0){
					if (dL_dchannel < 0){
						atomicAdd(&dL_dmask_shunts[global_id].x, dchannel_dcolor * dL_dchannel * scale);
						atomicAdd(&dL_dmask_signals[global_id].x, 1.0f);
					} else if(dL_dchannel > 0){
						atomicAdd(&dL_dmask_shunts[global_id].y, dchannel_dcolor * dL_dchannel * scale);
						atomicAdd(&dL_dmask_signals[global_id].y, 1.0f);
					}
				}
			}
	...
	bool antialiasing)
{
	...
	preprocessCUDA<NUM_CHANNELS> << < (P + 255) / 256, 256 >> > (
		...
		projmatrix,
		campos,
		(float2*)dL_dmask_shunts, // new
		(float2*)dL_dmask_signals, // new
		(float3*)dL_dmean2D,
		(glm::vec3*)dL_dmean3D,
		dL_dcolor,
		dL_dcov3D,
