	...
	const bool debug)
{
  ...
  torch::Tensor dL_dmask_shunts = torch::zeros({P, 2}, means3D.options()); // new
  torch::Tensor dL_dmask_signals = torch::zeros({P, 2}, means3D.options()); // new
  ...
  {  
	  ...
	  dL_dout_invdepthptr,
	  dL_dmask_shunts.contiguous().data<float>(), 		// new
	  dL_dmask_signals.contiguous().data<float>(), 	// new
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dmask_shunts, dL_dmask_signals); // new
}
